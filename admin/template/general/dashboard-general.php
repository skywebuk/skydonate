...
<div class="skyweb-settings-panel">
    <form class="skydonation-general-form" method="post" action="">
        ...
        <table class="form-table">
            <?php if(sky_setup_check('enable_sky_donations_module')): ?>
            <tr>
                <th scope="row"><label for="enable_sky_donations_module">Enable Sky Donations Module</label></th>
                <td>
                    <input type="checkbox" name="enable_sky_donations_module" id="enable_sky_donations_module" value="1" <?php checked(get_option('enable_sky_donations_module'), 1); ?> <?php echo esc_attr(LDIS); ?>>
                    <small>Enable or disable the Sky Donations module. Check this box to activate the module.</small>
                </td>
            </tr>
            <?php endif; ?>
            <?php if(sky_setup_check('enable_custom_login_form')): ?>
            <tr>
                <th scope="row"><label for="enable_custom_login_form">Enable Custom Login Form</label></th>
                <td>
                    <input type="checkbox" name="enable_custom_login_form" id="enable_custom_login_form" value="1" <?php checked(get_option('enable_custom_login_form'), 1); ?> <?php echo esc_attr(LDIS); ?>>
                    <small>Enable or disable a custom login form provided by the Sky Donations plugin.</small>
                </td>
            </tr>
            <?php endif; ?>
            <?php if(sky_setup_check('checkout_custom_field_style')): ?>
            <tr>
                <th scope="row"><label for="checkout_custom_field_style">Checkout Custom Field Style</label></th>
                <td>
                    <input type="checkbox" name="checkout_custom_field_style" id="checkout_custom_field_style" value="1" <?php checked(get_option('checkout_custom_field_style'), 1); ?> <?php echo esc_attr(LDIS); ?>>
                    <small>Check the styling option for custom fields during the checkout process.</small>
                </td>
            </tr>
            <?php endif; ?>
            <?php if(sky_setup_check('recent_donation_list_with_country')): ?>
            <tr>
                <th scope="row"><label for="recent_donation_list_with_country">Recent Donation List with Country Name and Flag</label></th>
                <td>
                    <input type="checkbox" name="recent_donation_list_with_country" id="recent_donation_list_with_country" value="1" <?php checked(get_option('recent_donation_list_with_country'), 1); ?> <?php echo esc_attr(LDIS); ?>>
                    <small>If enabled, you will add the functionality to include the Recent Donation List with country name and flag.</small>
                </td>
            </tr>
            <?php endif; ?>
            <?php if(sky_setup_check('auto_complete_processing')): ?>
            <tr>
                <th scope="row"><label for="auto_complete_processing">Auto Complete Processing</label></th>
                <td>
                    <input type="checkbox" name="auto_complete_processing" id="auto_complete_processing" value="1" <?php checked(get_option('auto_complete_processing'), 1); ?> <?php echo esc_attr(LDIS); ?>>
                    <small>If enabled, automatically complete donation payments on the Thank You page.</small>
                </td>
            </tr>
            <?php endif; ?>
            <?php if(sky_setup_check('enable_donation_goal')): ?>
            <tr>
                <th scope="row"><label for="enable_donation_goal">Enable Donation Goal</label></th>
                <td>
                    <input type="checkbox" name="enable_donation_goal" id="enable_donation_goal" value="1" <?php checked(get_option('enable_donation_goal'), 1); ?> <?php echo esc_attr(LDIS); ?>>
                    <small>If enabled, an option for a Donation Goal will be available for the donation appeals.</small>
                </td>
            </tr>
            <?php endif; ?>
            <?php if(sky_setup_check('enable_title_prefix')): ?>
            <tr>
                <th scope="row"><label for="enable_title_prefix">Enable Title Prefix</label></th>
                <td>
                    <input type="checkbox" name="enable_title_prefix" id="enable_title_prefix" value="1" <?php checked(get_option('enable_title_prefix'), 1); ?> <?php echo esc_attr(LDIS); ?>>
            ...
            <tr class="title_prefix_row" style="<?php echo get_option('enable_title_prefix') ? '' : 'display:none;'; ?>">
                ...
                <td>
                    <?php
                    // Ensure $selected_titles is an array
                    ...
                    <select name="select_title_prefix" class="select_type_items" id="select_title_prefix" 
                        <?php echo esc_attr(LDIS); ?> multiple="multiple" style="width: 100%; height: 150px;">
                        <?php
